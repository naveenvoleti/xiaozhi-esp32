...
#define TAG "SparkyEmojiDisplay"
SparkyEmojiDisplay::SparkyEmojiDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, int width, int height, int offset_x, int offset_y, bool mirror_x, bool mirror_y,
                                           bool swap_xy)
    : SpiLcdDisplay(panel_io, panel, width, height, offset_x, offset_y, mirror_x, mirror_y, swap_xy) {
    InitializeSparkyEmojis();
    SetupChatLabel();
}

void SparkyEmojiDisplay::InitializeSparkyEmojis() {
    ESP_LOGI(TAG, "Initialize Sparky GIF expression");
    ...
    ESP_LOGI(TAG, "Sparky Gif expression initialization completed");
}

void SparkyEmojiDisplay::SetupChatLabel() {
    ...
    lv_obj_set_width(chat_message_label_, width_ * 0.9);                        // Limit the width to the screen width 90%
    lv_label_set_long_mode(chat_message_label_, LV_LABEL_LONG_SCROLL_CIRCULAR);            // Set to word wrap mode
...
void SparkyEmojiDisplay::SetStatus(const char* status) {
